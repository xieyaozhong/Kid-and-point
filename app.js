...
import {
  ...
  onSnapshot, serverTimestamp, arrayUnion, runTransaction
...
function showOnly(id){
  ['loadingScreen','firebaseScreen','authScreen','onboardScreen','appScreen'].forEach(x => $('#'+x)?.classList.toggle('hidden', x !== id));
}
...
function notify(text, kind='ok'){
  const box=$('#modalBody');
  if(box && $('#modalWrap').classList.contains('open')) box.insertAdjacentHTML('afterbegin',`<div class="notice ${kind}">${esc(text)}</div>`);
}
...
function fieldStudents(){
  return state.students.map(s=>`<option value="${esc(s.id)}" ${s.id===selectedStudentId?'selected':''}>${esc(s.name)}</option>`).join('');
}
function randomCode(){
  const chars='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  const a=new Uint32Array(6); crypto.getRandomValues(a);
  return 'KP-' + [...a].map(n=>chars[n%chars.length]).join('');
}
function authError(e){
  ...
  return map[e?.code] || e?.message || '操作失敗';
}

function initConfigScreen(){
  const cfg=activeConfig();
  if(cfg && hasFirebaseConfig(cfg)) return initFirebase(cfg);
  showOnly('firebaseScreen');
}

$('#firebaseForm').addEventListener('submit', e=>{
  e.preventDefault();
  const f=new FormData(e.currentTarget);
  const cfg=Object.fromEntries([...f.entries()].map(([k,v])=>[k,String(v).trim()]));
...
function initFirebase(cfg){
  try{
    ...
    showOnly('firebaseScreen');
    alert('Firebase 設定無法初始化：'+e.message);
  }
}
...
async function handleAuthState(user){
  ...
  try{
    ...
    if(!profile){
      profile={name:user.email?.split('@')[0]||'使用者',email:user.email||'',roleHint:localStorage.getItem('kidpoint.roleHint')||'parent',familyId:null};
      await setDoc(doc(db,'users',user.uid),profile,{merge:true});
    }
    if(profile.familyId) await connectFamily(profile.familyId);
    else showOnboarding();
  }catch(e){
    console.error(e); showOnly('authScreen'); authMessage('讀取帳號資料失敗：'+e.message);
  }
}
...
function setAuthMode(mode){
  ...
  $('#roleField').classList.toggle('hidden',mode!=='register');
...
$$('.role-card').forEach(btn=>btn.onclick=()=>{
  registerRole=btn.dataset.role;
  $$('.role-card').forEach(x=>x.classList.toggle('active',x===btn));
});
$('#authForm').addEventListener('submit',async e=>{
  e.preventDefault(); authMessage('');
  const f=new FormData(e.currentTarget); const email=String(f.get('email')).trim(); const password=String(f.get('password'));
  $('#authSubmit').disabled=true;
  try{
    if(authMode==='login') await signInWithEmailAndPassword(auth,email,password);
    else{
      const name=String(f.get('name')||'').trim()||email.split('@')[0];
      localStorage.setItem('kidpoint.roleHint',registerRole);
      const cred=await createUserWithEmailAndPassword(auth,email,password);
      await setDoc(doc(db,'users',cred.user.uid),{name,email,roleHint:registerRole,familyId:null,createdAt:serverTimestamp()},{merge:true});
      profile={name,email,roleHint:registerRole,familyId:null};
      showOnboarding();
    }
...
function showOnboarding(){
  showOnly('onboardScreen');
  const hint=profile?.roleHint || localStorage.getItem('kidpoint.roleHint') || 'parent';
  $('#welcomeName').textContent=`${profile?.name||'你好'}，設定你的 Kid & Point`;
  $('#teacherOnboard').classList.toggle('hidden',hint!=='teacher');
  $('#parentOnboard').classList.toggle('hidden',hint!=='parent');
  onboardMessage('');
}
...
$('#createFamilyForm').addEventListener('submit',async e=>{
  e.preventDefault(); onboardMessage('');
  const name=String(new FormData(e.currentTarget).get('familyName')).trim(); if(!name)return;
  let code;
  try{
    for(let i=0;i<5;i++){
      code=randomCode(); const ref=doc(db,'families',code); const snap=await getDoc(ref); if(!snap.exists()) break;
    }
    await setDoc(doc(db,'families',code),{name,teachers:[currentUser.uid],parents:[],createdAt:serverTimestamp(),createdBy:currentUser.uid});
    await setDoc(doc(db,'users',currentUser.uid),{familyId:code,roleHint:'teacher',name:profile.name,email:currentUser.email},{merge:true});
    profile={...profile,familyId:code,roleHint:'teacher'};
    await connectFamily(code);
  }catch(err){ console.error(err); onboardMessage('建立家庭失敗：'+err.message); }
});

$('#joinFamilyForm').addEventListener('submit',async e=>{
  e.preventDefault(); onboardMessage('');
  const code=String(new FormData(e.currentTarget).get('familyCode')).trim().toUpperCase();
  try{
    const ref=doc(db,'families',code); const snap=await getDoc(ref);
    if(!snap.exists()) return onboardMessage('找不到這個家庭代碼，請向老師確認');
    await updateDoc(ref,{parents:arrayUnion(currentUser.uid)});
    await setDoc(doc(db,'users',currentUser.uid),{familyId:code,roleHint:'parent',name:profile.name,email:currentUser.email},{merge:true});
    profile={...profile,familyId:code,roleHint:'parent'};
    await connectFamily(code);
  }catch(err){ console.error(err); onboardMessage('加入家庭失敗：'+err.message); }
});

async function connectFamily(id){
  cleanupListeners(); familyId=id;
  const famRef=doc(db,'families',id); const famSnap=await getDoc(famRef);
  if(!famSnap.exists()){
    await setDoc(doc(db,'users',currentUser.uid),{familyId:null},{merge:true}); profile.familyId=null; return showOnboarding();
  }
  state.family={id,...famSnap.data()};
  role=state.family.teachers?.includes(currentUser.uid)?'teacher':state.family.parents?.includes(currentUser.uid)?'parent':null;
  if(!role){
    await setDoc(doc(db,'users',currentUser.uid),{familyId:null},{merge:true}); profile.familyId=null; return showOnboarding();
  }
  showOnly('appScreen');
  $('#familyName').textContent=state.family.name||'Kid & Point';
  $('#userName').textContent=profile?.name||currentUser.email;
  $('#rolePill').textContent=role==='teacher'?'老師端':'家長端';
  $('#teacherView').classList.toggle('hidden',role!=='teacher');
  $('#parentView').classList.toggle('hidden',role!=='parent');
  $('#familyCodeWrap').classList.toggle('hidden',role!=='teacher');
  $('#familyCode').textContent=id;
  listen(famRef,'family');
  ['students','tasks','submissions','rewards','redemptions','feedback','ledger'].forEach(name=>listen(collection(db,'families',id,name),name));
}

function listen(ref,key){
  const unsub=onSnapshot(ref,snap=>{
    if(key==='family'){
      state.family={id:snap.id,...snap.data()}; $('#familyName').textContent=state.family.name||'Kid & Point'; return;
    }
    ...
    render();
    $('#syncDot').classList.add('online'); $('#syncText').textContent='Firestore 即時同步中';
  },err=>{
    console.error(key,err); $('#syncDot').classList.remove('online'); $('#syncText').textContent='同步失敗：'+err.code;
  });
  unsubscribers.push(unsub);
}
function cleanupListeners(){ unsubscribers.forEach(f=>f()); unsubscribers=[]; }

$('#familyCodeWrap').onclick=async()=>{
  if(!familyId)return; try{await navigator.clipboard.writeText(familyId); $('#familyCodeWrap').innerHTML=`已複製：<b class="copy-code">${esc(familyId)}</b>`; setTimeout(()=>{$('#familyCodeWrap').innerHTML=`家庭代碼：<b id="familyCode" class="copy-code">${esc(familyId)}</b>`;},1500);}catch{}
};
$('#studentSelect').addEventListener('change',e=>{selectedStudentId=e.target.value;render();});

function render(){
  const sel=$('#studentSelect');
  sel.innerHTML=state.students.length?state.students.map(s=>`<option value="${esc(s.id)}" ${s.id===selectedStudentId?'selected':''}>${esc(s.name)}</option>`).join(''):'<option value="">尚無學生</option>';
  sel.disabled=!state.students.length;
  renderResources();
  role==='teacher'?renderTeacher():renderParent();
}
function metrics(s){
  if(!s)return {today:0,week:0,done:0,pending:0};
  const ledger=state.ledger.filter(x=>x.studentId===s.id);
  const today=ledger.filter(x=>tsValue(x.createdAt)>=startToday()).reduce((a,x)=>a+Number(x.amount||0),0);
  const week=ledger.filter(x=>tsValue(x.createdAt)>=startWeek()).reduce((a,x)=>a+Number(x.amount||0),0);
  const done=state.tasks.filter(x=>x.studentId===s.id&&x.status==='done').length;
  const pending=state.submissions.filter(x=>x.studentId===s.id&&x.status==='pending').length;
  return {today,week,done,pending};
}
function renderTeacher(){
  const s=student(),m=metrics(s),pts=Number(s?.points||0);
  $('#tName').textContent=s?.name||'尚未新增學生'; $('#tPoints').textContent=pts; $('#tProgress').style.width=Math.min(100,Math.max(0,m.week))+'%'; $('#weekText').textContent=`${m.week} / 100`; $('#todayEarn').textContent=m.today; $('#doneCount').textContent=m.done; $('#pendingCount').textContent=m.pending;
  const ledger=state.ledger.filter(x=>x.studentId===s?.id).sort((a,b)=>tsValue(b.createdAt)-tsValue(a.createdAt)).slice(0,8);
  $('#teacherHistory').innerHTML=ledger.length?ledger.map(l=>`<div class="history-item"><div><b>${esc(l.reason)}</b><div class="meta">${fmt(l.createdAt)}</div></div><span class="${Number(l.amount)>=0?'pos':'neg'}">${Number(l.amount)>=0?'+':''}${Number(l.amount)}</span></div>`).join(''):'<div class="empty">尚無點數異動</div>';
  renderTeacherList(s);
}
function renderTeacherList(s){
  ...
  if(teacherTab==='active'){
    const arr=state.tasks.filter(t=>t.studentId===s.id&&t.status==='active').sort((a,b)=>tsValue(b.createdAt)-tsValue(a.createdAt));
    out.innerHTML=arr.length?arr.map(t=>taskRow(t,'teacher')).join(''):'<div class="empty">目前沒有進行中的任務</div>';
  }else if(teacherTab==='verify'){
    const arr=state.submissions.filter(x=>x.studentId===s.id&&x.status==='pending').sort((a,b)=>tsValue(b.submittedAt)-tsValue(a.submittedAt));
    out.innerHTML=arr.length?arr.map(sub=>{const t=taskById(sub.taskId);return `<div class="task"><div class="icon">✓</div><div class="task-main"><h4>${esc(t?.title||'任務')}</h4><div class="meta">回報：${fmt(sub.submittedAt)} ${sub.note?`｜${esc(sub.note)}`:''}</div><span class="badge">待核實</span> <span class="pt">+${Number(t?.points||0)} 點</span></div><div><button class="btn green" data-approve-sub="${esc(sub.id)}">通過</button> <button class="btn danger" data-reject-sub="${esc(sub.id)}">退回</button></div></div>`}).join(''):'<div class="empty">目前沒有待核實任務</div>';
...
function taskRow(t,mode){
  return `<div class="task"><div class="icon">${t.category==='健康作息'?'🌱':'📚'}</div><div class="task-main"><h4>${esc(t.title)}</h4><div><span class="badge">${esc(t.category||'任務')}</span><span class="badge">${esc(t.subCategory||'')}</span>${t.dueAt?`<span class="badge">期限 ${esc(t.dueAt)}</span>`:''}</div><div class="meta" style="margin-top:6px">${mode==='done'?'核實完成 '+fmt(t.verifiedAt):'發布 '+fmt(t.createdAt)}</div></div><span class="pt">+${Number(t.points||0)} 點</span></div>`;
}
function renderParent(){
  const s=student(),m=metrics(s),pts=Number(s?.points||0); $('#pName').textContent=s?.name||'尚未建立學生'; $('#pPoints').textContent=pts; $('#pProgress').style.width=Math.min(100,Math.max(0,m.week))+'%'; $('#pWeekText').textContent=`本週 ${m.week} 點`; renderParentList(s);
}
function renderParentList(s){
  const out=$('#parentList'); if(!s){out.innerHTML='<div class="empty">老師尚未建立學生資料</div>';return;}
  if(parentTab==='tasks'){
    const arr=state.tasks.filter(t=>t.studentId===s.id&&t.status==='active').sort((a,b)=>tsValue(b.createdAt)-tsValue(a.createdAt));
    out.innerHTML=arr.length?arr.map(t=>{
      const sub=state.submissions.filter(x=>x.taskId===t.id&&x.studentId===s.id).sort((a,b)=>tsValue(b.submittedAt)-tsValue(a.submittedAt))[0];
      const action=sub?.status==='pending'?'<span class="badge">已回報・待核實</span>':`<button class="btn green" data-submit-task="${esc(t.id)}">回報完成</button>`;
      return `<div class="task"><div class="icon">${t.category==='健康作息'?'🌱':'📚'}</div><div class="task-main"><h4>${esc(t.title)}</h4><span class="badge">${esc(t.category)}</span><span class="badge">${esc(t.subCategory||'')}</span><div class="meta" style="margin-top:6px">完成可獲得 <b>+${Number(t.points||0)}</b> 點${sub?.status==='rejected'?' · 上次回報已退回':''}</div></div>${action}</div>`;
    }).join(''):'<div class="empty">目前沒有需要完成的任務</div>';
...
function bindTabs(containerId, setter){
  $('#'+containerId).addEventListener('click',e=>{const b=e.target.closest('[data-tab]');if(!b)return;$('#'+containerId).querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===b));setter(b.dataset.tab);render();});
}
bindTabs('teacherTabs',x=>teacherTab=x); bindTabs('parentTabs',x=>parentTab=x);

$('#teacherList').addEventListener('click',async e=>{
  const a=e.target.closest('[data-approve-sub]'),r=e.target.closest('[data-reject-sub]');
  if(a) await approveSubmission(a.dataset.approveSub);
  if(r) await rejectSubmission(r.dataset.rejectSub);
});
$('#parentList').addEventListener('click',e=>{
  const submit=e.target.closest('[data-submit-task]'), redeem=e.target.closest('[data-redeem]');
  if(submit) openSubmitModal(submit.dataset.submitTask);
  if(redeem) requestRedeem(redeem.dataset.redeem);
});

document.addEventListener('click',e=>{
  const trigger=e.target.closest('[data-open]'); if(!trigger)return;
  const type=trigger.dataset.open;
  if(type==='student') openStudentModal(); if(type==='task') openTaskModal(); if(type==='quickPoint') openPointModal(); if(type==='redemptions') openRedemptionsModal(); if(type==='reward') openRewardModal(); if(type==='feedback') openFeedbackModal();
});
$('#modalWrap').addEventListener('click',e=>{if(e.target===$('#modalWrap')||e.target.closest('[data-close]'))closeModal();});

function modalShell(title,body,submit='儲存'){
  return `<div class="row between"><h3>${esc(title)}</h3><button class="btn secondary" data-close type="button">關閉</button></div>${body}<div class="actions"><button class="btn secondary" type="button" data-close>取消</button><button class="btn green" type="submit">${esc(submit)}</button></div>`;
}
function openStudentModal(){
  openModal(`<form id="studentForm">${modalShell('新增學生','<div class="field"><label>孩子姓名</label><input name="name" required placeholder="宇傑"></div><div class="field"><label>年級（選填）</label><input name="grade" placeholder="小六"></div>','建立學生')}</form>`);
  $('#studentForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);try{const ref=await addDoc(collection(db,'families',familyId,'students'),{name:String(f.get('name')).trim(),grade:String(f.get('grade')).trim(),points:0,createdAt:serverTimestamp()});selectedStudentId=ref.id;closeModal();}catch(err){notify(err.message,'bad')}};
}
function openTaskModal(){
  if(!state.students.length)return openStudentModal();
  const opts=builtins.map((x,i)=>`<option value="${i}">${esc(x.category)}｜${esc(x.title)} (+${x.points})</option>`).join('');
...
function openPointModal(){
  if(!state.students.length)return openStudentModal();
  openModal(`<form id="pointForm">${modalShell('即時加／扣點',`<div class="field"><label>學生</label><select name="studentId">${fieldStudents()}</select></div><div class="field"><label>原因</label><input name="reason" required placeholder="作業表現傑出"></div><div class="field"><label>點數（扣點請輸入負數）</label><input name="amount" type="number" min="-500" max="500" value="10" required></div>`,'確認記錄')}</form>`);
  $('#pointForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);try{await adjustPoints(String(f.get('studentId')),Number(f.get('amount')),String(f.get('reason')).trim(),'manual');closeModal();}catch(err){notify(err.message,'bad')}};
}
function openRewardModal(){
  if(!student())return;
  openModal(`<form id="rewardForm">${modalShell('新增獎品',`<div class="field"><label>獎品名稱</label><input name="title" required placeholder="週末多 30 分鐘遊戲時間"></div><div class="field"><label>需要點數</label><input name="cost" type="number" min="1" value="50" required></div><div class="field"><label>補充說明</label><textarea name="description" placeholder="可和孩子一起約定使用方式"></textarea></div>`,'新增獎品')}</form>`);
  $('#rewardForm').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget);try{await addDoc(collection(db,'families',familyId,'rewards'),{studentId:selectedStudentId,title:String(f.get('title')).trim(),cost:Number(f.get('cost')),description:String(f.get('description')).trim(),active:true,createdAt:serverTimestamp(),createdBy:currentUser.uid});closeModal();parentTab='rewards';render();}catch(err){notify(err.message,'bad')}};
}
function openFeedbackModal(){
  if(!student())return;
  openModal(`<form id="feedbackForm">${modalShell('記錄孩子回饋',`<div class="field"><label>今天的觀察、孩子說的話或心得</label><textarea name="content" required placeholder="今天做功課比較專心，也主動說想把喝水任務完成。"></textarea></div>`,'記錄')}</form>`);
  $('#feedbackForm').onsubmit=async e=>{e.preventDefault();const content=String(new FormData(e.currentTarget).get('content')).trim();try{await addDoc(collection(db,'families',familyId,'feedback'),{studentId:selectedStudentId,content,authorUid:currentUser.uid,authorRole:role,createdAt:serverTimestamp()});closeModal();if(role==='parent')parentTab='feedback';render();}catch(err){notify(err.message,'bad')}};
}
function openSubmitModal(taskId){
  const t=taskById(taskId); if(!t)return;
  openModal(`<form id="submitTaskForm">${modalShell('回報任務完成',`<div class="notice">回報後不會立刻加點，要由老師核實才會正式獲得 <b>${Number(t.points||0)} 點</b>。</div><h3 style="font-size:16px">${esc(t.title)}</h3><div class="field"><label>補充說明（選填）</label><textarea name="note" placeholder="例如：今天喝了 1600ml"></textarea></div>`,'送出回報')}</form>`);
  $('#submitTaskForm').onsubmit=async e=>{e.preventDefault();const note=String(new FormData(e.currentTarget).get('note')).trim();try{await addDoc(collection(db,'families',familyId,'submissions'),{taskId,studentId:t.studentId,status:'pending',note,submittedAt:serverTimestamp(),submittedBy:currentUser.uid});closeModal();}catch(err){notify(err.message,'bad')}};
}
function openRedemptionsModal(){
  const arr=state.redemptions.filter(x=>x.status==='pending').sort((a,b)=>tsValue(b.requestedAt)-tsValue(a.requestedAt));
  const body=arr.length?arr.map(x=>{const r=rewardById(x.rewardId),s=state.students.find(v=>v.id===x.studentId);return `<div class="task"><div class="icon reward">🎁</div><div class="task-main"><h4>${esc(r?.title||'獎品')}</h4><div class="meta">${esc(s?.name||'學生')} · ${Number(r?.cost||0)} 點 · ${fmt(x.requestedAt)}</div></div><button class="btn green" data-approve-redemption="${esc(x.id)}">核實兌換</button></div>`}).join(''):'<div class="empty">目前沒有待核實兌換</div>';
  openModal(`<div class="row between"><h3>待核實兌換</h3><button class="btn secondary" data-close>關閉</button></div>${body}`);
  $$('[data-approve-redemption]').forEach(b=>b.onclick=async()=>{try{await approveRedemption(b.dataset.approveRedemption);closeModal();}catch(err){notify(err.message,'bad')}});
}

async function adjustPoints(studentId,amount,reason,source){
  if(!amount)throw new Error('點數不能是 0');
  const sRef=doc(db,'families',familyId,'students',studentId); const lRef=doc(collection(db,'families',familyId,'ledger'));
...
async function approveSubmission(id){
  ...
  await runTransaction(db,async tx=>{
    const subSnap=await tx.get(subRef);if(!subSnap.exists()||subSnap.data().status!=='pending')throw new Error('這筆回報已處理');const sub=subSnap.data();
    const tRef=doc(db,'families',familyId,'tasks',sub.taskId),sRef=doc(db,'families',familyId,'students',sub.studentId);const tSnap=await tx.get(tRef);const sSnap=await tx.get(sRef);if(!tSnap.exists()||!sSnap.exists())throw new Error('任務或學生資料不存在');const t=tSnap.data(),amount=Number(t.points||0),lRef=doc(collection(db,'families',familyId,'ledger'));
